...
# @File          : transmission
# @Description   : transmission docker container installer
# @TODO          :
# @Other         :
# @Resource      :
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
TRANSMISSION_SERVER_PORT="${TRANSMISSION_SERVER_PORT:-9091}"
TRANSMISSION_SERVER_HOST="${TRANSMISSION_SERVER_HOST:-$(hostname -f 2>/dev/null)}"
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/$APPNAME"
REPO_BRANCH="${GIT_REPO_BRANCH:-main}"
TRANSMISSION_SERVER_TIMEZONE="${TZ:-${TIMEZONE:-America/New_York}}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__sudo mkdir -p "$DATADIR/downloads"
__sudo mkdir -p "$DATADIR/watch"
__sudo mkdir -p "$DATADIR/torrents"
...
if docker ps -a | grep -qs "$APPNAME"; then
  printf_blue "Service is available at: http://$TRANSMISSION_SERVER_HOST:$TRANSMISSION_SERVER_PORT"
else
  false
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
